...
doparse(int (*ifunc) (void), char *srctypename, char *srcname)
{
  ...
  if (ParseError)
  {
    fprintf(stderr, "%s:  errors found in twm %s", ProgramName, srctypename);
    if (srcname)
...
#ifndef NO_M4_SUPPORT
int
ParseTwmrc(char *filename, char *display_name, int m4_preprocess, char *m4_option)
#else
int
...
#endif
{
  ...
  if (twmrc)
  {
    ...
    {
      fprintf(stderr, "%s:  unable to open twmrc file %s, using %s instead\n", ProgramName, filename, cp);
    }
  ...
  else
  {
    if (filename)
    {
      fprintf(stderr, "%s:  unable to open twmrc file %s, using built-in defaults instead\n", ProgramName, filename);
    }
...
static TwmKeyword keytable[] = {
  ...
  {"f.deletedoorbyname", FSKEYWORD, F_DELETEDOORBYNAME},
  ...
  {"f.enterdoorbackwards", FKEYWORD, F_ENTERDOORBACKWARDS},
  ...
  {"f.functionifexitvaleq", FS2KEYWORD, F_FUNCTIONIFEXITVALEQ},
  {"f.functionifexitvaleqelse", FS3KEYWORD, F_FUNCTIONIFEXITVALEQELSE},
  ...
  {"f.newdoor3", FS3KEYWORD, F_NEWDOOR3},
  {"f.newdoor4", FS4KEYWORD, F_NEWDOOR4},
  ...
  {"f.returnifclipeq", FSKEYWORD, F_RETURNIFCLIPEQ},
  {"f.returnifclipneq", FSKEYWORD, F_RETURNIFCLIPNEQ},
  {"f.returnifexitvaleq", FSKEYWORD, F_RETURNIFEXITVALEQ},
  {"f.returnifexitvalneq", FSKEYWORD, F_RETURNIFEXITVALNEQ},
  {"f.returnifretvalnok", FKEYWORD, F_RETURNIFRETVALNOK},
  {"f.returnifretvalok", FKEYWORD, F_RETURNIFRETVALOK},
...
put_pixel_on_root(Pixel pixel)
{
  ...
  if (XGetWindowProperty(dpy, Scr->Root, pixelAtom, 0, 8192,
			 ...
			 &retFormat, &nPixels, &retAfter, (unsigned char **)&retProp) != Success || retAtom == None)
    return;
  ...
  if (addPixel)
    XChangeProperty(dpy, Scr->Root, _XA_MIT_PRIORITY_COLORS, XA_CARDINAL, 32, PropModeAppend, (unsigned char *)&pixel, 1);
}
...
make_m4_cmdline(char *display_name, char *cp, char *m4_option)
{
   ...
   */
  for (i = 0; i < 6; i++)
    cmd_len += strlen(m4_lines[i]);
  cmd_len += M4_MAXDIGITS * 9 + HomeLen + strlen(vc) + strlen(is_xpm) +
    strlen(is_color) + strlen(env_username) + strlen(server) +
    strlen(client) + strlen(hostname) + strlen(ServerVendor(dpy)) + strlen(is_sound) + strlen(is_regex) + strlen(cp) + 16;
  if (opt_len)
    cmd_len += opt_len;
