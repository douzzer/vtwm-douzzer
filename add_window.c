...
AddMoveAndResize(TwmWindow * tmp_win, int ask_user)
{
  ...
  if (HandlingEvents && ask_user)
  {
    ...
    {
      /* find pointer */
      if (!XQueryPointer(dpy, Scr->Root, &JunkRoot, &JunkChild, &JunkX, &JunkY, &AddingX, &AddingY, &JunkMask))
	JunkMask = 0;
    ...
    {				/* else prompt */
      if (!(tmp_win->wmhints && tmp_win->wmhints->flags & StateHint && tmp_win->wmhints->initial_state == IconicState))
      {
	Bool firsttime = True;
	...
	while (TRUE)
	{
	  ...
	  JunkMask &= (Button1Mask | Button2Mask | Button3Mask | Button4Mask | Button5Mask);
...
#undef HALF_AVE_CURSOR_SIZE
	  ...
	  {
	    XWarpPointer(dpy, None, Scr->Root, 0, 0, 0, 0, AddingX + AddingW / 2, AddingY + AddingH / 2);
	  }
	  ...
	  while (TRUE)
	  {
	    XMaskEvent(dpy, ButtonReleaseMask | ButtonMotionMask, &event);
	...
	tmp_win->attr.y = AddingY + tmp_win->title_height + tmp_win->frame_bw + tmp_win->frame_bw3D;

	tmp_win->attr.width = AddingW - bw2 - 2 * tmp_win->frame_bw3D;
	tmp_win->attr.height = AddingH - tmp_win->title_height - bw2 - 2 * tmp_win->frame_bw3D;
  ...
  tmp_win->frame_x = tmp_win->attr.x - tmp_win->frame_bw - tmp_win->frame_bw3D;
  tmp_win->frame_y = tmp_win->attr.y - tmp_win->title_height - tmp_win->frame_bw - tmp_win->frame_bw3D;
  ...
  fprintf(stderr, "  position window  %d, %d  %dx%d\n",
	  tmp_win->attr.x, tmp_win->attr.y, tmp_win->attr.width, tmp_win->attr.height);
#endif
}
...
GrabModKeys(Window w, FuncKey * k)
{
  ...
  for (i = 1; i <= Scr->IgnoreModifiers; i++)
    if ((Scr->IgnoreModifiers & i) == i)
      XGrabKey(dpy, k->keycode, k->mods | i, w, True, GrabModeAsync, GrabModeAsync);
}
...
CreateHighlightWindow(TwmWindow * tmp_win)
{
  ...
  if (!Scr->hilitePm)
  {
	...
	Scr->hilitePm->pixmap = XCreateBitmapFromData(dpy,
						      tmp_win->title_w.win, gray_bits, Scr->hilitePm->width, Scr->hilitePm->height);
    }
  }
...
#ifndef NO_XPM_SUPPORT
  pm_numcolors = SetPixmapsBackground(Scr->hilitePm, Scr->Root, tmp_win->title.back);
#endif
  ...
  if (Scr->hilitePm->pixmap)
  {
    ...
    {
      pm = XCreatePixmap(dpy, tmp_win->title_w.win, Scr->hilitePm->width, Scr->hilitePm->height, Scr->d_depth);
	...
	XCopyPlane(dpy, Scr->hilitePm->pixmap, pm, gc, 0, 0, Scr->hilitePm->width, Scr->hilitePm->height, 0, 0, 1);
...
ComputeCommonTitleOffsets(void)
{
  ...
  Scr->TBInfo.titlex = (Scr->TBInfo.leftx + (Scr->TBInfo.nleft * buttonwidth) - Scr->TBInfo.pad + en);

  if (Scr->TBInfo.nright > 0)
    Scr->TBInfo.rightoff += (Scr->ButtonIndent + ((Scr->TBInfo.nright * buttonwidth) - Scr->TBInfo.pad));
  return;
}
...
ComputeWindowTitleOffsets(TwmWindow * tmp_win, int width, Bool squeeze)
{
  ...
  {
    int rx = tmp_win->highlightx + ((tmp_win->titlehighlight) ? Scr->TitleHeight * 2 : 0);
...
CreateWindowTitlebarButtons(TwmWindow * tmp_win)
{
  ...
  if (nb > 0)
  {
    ...
    {
      for (tb = Scr->TBInfo.head, tbw = tmp_win->titlebuttons; tb; tb = tb->next, tbw++)
      {
	if (tb->rightside)
	{
	  x = rightx;
	...
	tbw->window = XCreateWindow(dpy, tmp_win->title_w.win, x, y, h, h,
				    ...
				    0, (unsigned int)CopyFromParent, (Visual *) CopyFromParent, valuemask, &attributes);
  ...
  tmp_win->hilite_w = (tmp_win->titlehighlight && !Scr->hiliteName) ? CreateHighlightWindow(tmp_win) : None;
...
SetHighlightPixmap(char *filename)
{
  if (filename[0] == ':')
   ...
   if (!Scr->hilitePm)
    Scr->hilitePm = SetPixmapsPixmap(filename);
}
...
CreateColormapWindow(Window w, Bool creating_parent, Bool property_window)
{
  ...
  if (cwin)
  {
    if (!XGetWindowAttributes(dpy, w, &attributes) || XSaveContext(dpy, w, ColormapContext, (caddr_t) cwin))
    {
      free((char *)cwin);
      return (NULL);
    }

    if (XFindContext(dpy, attributes.colormap, ColormapContext, (caddr_t *) & cwin->colormap) == XCNOENT)
    {
     ...
     */
    cwin->visibility = creating_parent ? VisibilityPartiallyObscured : VisibilityUnobscured;
    cwin->refcnt = 1;
     ...
     */
    if (property_window &&
	(attributes.your_event_mask & (ColormapChangeMask | VisibilityChangeMask)) != (ColormapChangeMask | VisibilityChangeMask))
    {
      XSelectInput(dpy, w, attributes.your_event_mask | (ColormapChangeMask | VisibilityChangeMask));
    }
  }
...
FetchWmColormapWindows(TwmWindow * tmp)
{
  ...
  if (XGetWMColormapWindows(dpy, tmp->w, &cmap_windows, &number_cmap_windows) && number_cmap_windows > 0)
  {
    ...
    {				/* not in list */
      Window *new_cmap_windows = (Window *) malloc(sizeof(Window) * (number_cmap_windows + 1));

      if (!new_cmap_windows)
      {
	fprintf(stderr, "%s:  unable to allocate %d element colormap window array\n", ProgramName, number_cmap_windows + 1);
	goto done;
      }
    ...
    cwins = (ColormapWindow **) malloc(sizeof(ColormapWindow *) * number_cmap_windows);
    if (cwins)
    {
      for (i = 0; i < number_cmap_windows; i++)
      {
	...
	{
	  if (XFindContext(dpy, cmap_windows[i], ColormapContext, (caddr_t *) & cwins[i]) == XCNOENT)
	  {
	    if ((cwins[i] = CreateColormapWindow(cmap_windows[i], (Bool) tmp->cmaps.number_cwins == 0, True)) == NULL)
	    {
	      int k;
  ...
  if (number_cmap_windows > 1)
    tmp->cmaps.scoreboard = (char *)calloc(1, ColormapsScoreboardLength(&tmp->cmaps));
  ...
  if (!(supplied & PWinGravity) && (tmp->hints.flags & USPosition))
  {
    ...
    tmp->hints.win_gravity =
      gravs[((Scr->MyDisplayHeight - bottom <
	    ...
	    ((Scr->MyDisplayWidth - right < tmp->title_height + 2 * tmp->frame_bw3D) ? 0 : 1)];
